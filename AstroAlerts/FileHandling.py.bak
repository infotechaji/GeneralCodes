"""
    ...
    History    :
                ...
                v1.5 - 02/26/2019 - Destructor added to the class FileHandling
    Open Issues: None.
    Pending : Clean up
"""
...
class FileHandling():
    ...
    def __del__(self):
        #print ("FileHandling : object deleted")
        pass
...
if __name__ == '__main__':
    if not True:
        f_h=FileHandling()
        all_files = f_h.get_all_files(directory_is='D:\\_data\\product-biz-companies\\staging\\data\\',pattern='*core_ui_company_reference_links.psql',full_path=False)
        cnt = 0
        for each_file in all_files:
            cnt += 1
            print str(cnt) + '.\t',each_file
            f_h.rename_file(each_file,target_folder="D:\\_data\\product-biz-companies")
        exit()
    elif not True:
        f_h=FileHandling()
        ...
        f_h=FileHandling()
        input_file_name='D:\\_data\\smbnew_accounts_data\\unicode.post_feeds_social_media_trans1.txt'
        f_h.set_file_name(input_file_name)#'f3_news.txt')
        f_h.split_file(no_of_lines=600000,file_suffix='-QBS-split-')
        exit()
    elif not True:
        f_h=FileHandling()
