...
class AstroDeccan():
	...
	def get_page_source(self):
		...
		try:
			...
			#return url_content
		return url_content
	def get_prediction(self,my_astro=['all'],destroy_time=5,desktop_notification=True):
		...
		for each_div in div_tags:
			for each_astro in my_astro: # input list
				#print 'each_astro :',each_astro
				temp_dict={'astro':each_astro,'astro_prediction':''}
		...
		if desktop_notification==True:return self.display_prediction(temp_list,destroy_time)
...
if __name__ =='__main__':
	...
	my_astro=astr_obj.get_prediction(desktop_notification=False)
	count=0
	#print my_astro
	for i in my_astro:
		count+=1
