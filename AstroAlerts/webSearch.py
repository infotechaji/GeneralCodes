...
def fetch_for_url(actual_url,time_out_in=10,read_from_file=True,developer_mode=False,iteration=1,unescape=True,file_only_mode=False,translate=False):
    ...
    try:
        ...
        #print 'Url content length :',len(url_content)
...
def get_news_feeds_small(companyname,google_ned_country_code,print_instance=None,scoring=None,iteration=1):
    ...
    if country_code not in ('au','us'):#in uk ca nz en_za(SA) en_sg(sing)
...
def get_language_specific_url(page_content,language_key=['en','english','eng']):
    if not page_content :
        return''
    else:
        ...
        for each_a_tag in a_tags:
            if each_a_tag.text.strip(' \t\n\r').lower() in language_key:
                a_tag_text=each_a_tag.text
                href=each_a_tag.get('href')
                if href:
                    return {'href':href,'text':a_tag_text}
