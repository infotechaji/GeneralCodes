...
def get_host_details(): # gets current user name,host_name,ip_address 
    ...
    return {'host_name':host_name,
            ...
            'user_name':user_name}
def get_printable_time_stamp(get_exact_time=False,skip_seconds=False,get_date_only=False):
    c_time= str(datetime.datetime.now())
    date_time=c_time.split('.')[0]
    #print 'date_time :',date_time
    if get_date_only:return get_replaced_content(date_time.split()[0])
    if skip_seconds:
...
if __name__=="__main__":
    print get_printable_time_stamp(get_date_only=True)