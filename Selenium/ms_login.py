	...
	def ClickEachPage(self,directory_to_look=''):
		...
		for each_tag in divs:
			...
			try:
				...
				parent_div = each_tag.find_element_by_xpath('..')
				first_div=parent_div.find_elements_by_tag_name('div')[1]
				#print 'first_div :',first_div
				# span_text=''
				# for each_span in spans:
				# 	span_text+=each_span.text.strip()
				span_text=first_div.text.strip()
				# try:print 'span_text :',span_text
				# except: print 'span_text encode(utf-8):',span_text.encode('utf-8')
#print 'a_tag :',str(a_tag)
				
				#if parent_text:
				
				temp_person_name=person_name+'_'+span_text
				...
				#print  'directory_to_look :',directory_to_look
				file_exists=False
				if directory_to_look:
					directories=directory_to_look.split(',')
					#print 'directories to look:',len(directories)
					#print 'directories :',directories
					for each_directory in directories:
						temp_full_file_name =os.path.join(each_directory,temp_file_name)
						#print 'temp_full_file_name:',temp_full_file_name
						if path.exists(temp_full_file_name):
							print 'File exists'
							file_exists=True
							break
							# try:
							# 	copytree(each_directory, self.collected_path,files_list=[temp_file_name])
							# 	print 'copied'
							# 	break
							# except Exception as e :
							# 	print 'Exception while copying existing file ',e
							# 	pass
				...
				full_file_name =os.path.join(self.collected_path,temp_file_name)
				if path.exists(full_file_name) or file_exists==True : continue
				count+=1
				...
				pass
			# 	time.sleep(10)
	def click_next_page(self,wait_seconds=3):
		...
		for each_a_tag in a_tags:
			try:
				...
				print 'Exception in clicking next page element ',e
				if try_count>=2:return False
				pass
		self.error_count+=1
		...
		while True:
			...
			if count==1:
				...
				for i in range(next_page_limit):
					ms_obj.click_next_page(wait_seconds=0)
					sys.stdout.write('Next page clicked :'+str(i)+'Balance'+str(next_page_limit-i)+' \r')
			print 'Next page automation completed '
