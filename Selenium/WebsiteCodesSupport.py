"""
    ...
    History    :
                ...
                v2.11 - 07/14/2017 - Country code top level domain cannot be second top level domain. Made the change in the code.
                v2.12 - 01/01/2019 - Added second_level_suffix_before_country
    ...
    Pending :    For country specific websites parsing
                   ...
                   second level domains - https://en.wikipedia.org/wiki/Second-level_domain
                   ...
                   Handling IP address: http://185.15.169.148/web_asi/
                    ...
                    underscore usage in website name: http://www.aircraft_co.avic.com/
                    if I give wwg.co.in as input getting co.in as output which is wrong
"""
from Utilities import *
second_level_suffix_before_country=['ac','sch','nhs','gov','uk','nic','kommune']
...
country_code_top_level_domain=['ad','ae','af','ag','ai','al','am','an','ao','aq','ar','arpa','as','at','au','aw','az'
    ,'ba','bb','bd','be','bf','bg','bh','bi','bj','bm','bn','bo','br','bs','bt','bv','bw','by','bz'
    ,'ca','cc','cf','cg','ch','ci','ck','cl','cm','cn','co','cr','cs','cu','cv','cx','cy','cz'
    ,'de','dj','dk','dm','do','dz','ec','ee','eg','eh','er','es','et','fi','fj','fk','fm','fo','fr','fx'
    ,'ga','gb','gd','ge','gf','gh','gi','gl','gm','gn','gp','gq','gr','gs','gt','gu','gw','gy','hk','hm','hn','hr','ht','hu'
...
class WebURLParse():
    ...
    def process_website_suffix(self):
        ...
        for i in range(website_url_formatted_split_length):
            ...
            if suffix_found:
                if current_suffix in website_suffix_code_list and website_url_formatted_split_length>2 and (suffix_text in country_code_top_level_domain or current_suffix not in country_code_top_level_domain):
                    if self.developer_mode: 
                ...
                elif website_url_formatted_split_length > 2 and suffix_text in country_code_top_level_domain and current_suffix in second_level_suffix_before_country:
                    if self.developer_mode: 
                        self._print_(print_prefix + 'Website:' + str(self.website_url) + '\tInside 2nd suffix before country - ' + str(current_suffix))
                    suffix_text=current_suffix + '.' + suffix_text
                break
            else:
                try:
...
if __name__ == '__main__':
    if True:
        #web_ins=WebURLParse('https://and.nic.in/',ignore_errors=False,developer_mode=True)
        #web_ins=WebURLParse('http://www.iitk.ac.in/',ignore_errors=False,developer_mode=True)
        web_ins=WebURLParse('http://www.ndcn.ox.ac.uk/team/angela-vincent',ignore_errors=False,developer_mode=True)
        print  web_ins.get_website_domain_name() ,'\t Parent:' + web_ins.get_website_parent(),'\t Suffix:' ,web_ins.get_website_suffix()
        print "web_ins.get_website_parent(),'\t Suffix:' ,web_ins.get_website_suffix():",web_ins.get_website_parent()+'.'+web_ins.get_website_suffix()
        exit()
    if not True:
        web_ins=WebURLParse('https://w3d.4dhealthware.com/',ignore_errors=False,developer_mode=True)
        print  web_ins.get_website_domain_name() ,'\t Parent:' + web_ins.get_website_parent(),'\t Suffix:' ,web_ins.get_website_suffix()
        exit()
    if not True:
        ...
        link_list=[
                    'https://jobs.pseg.com/'
                    ,'http://career.gm.com'
                    ,'https://gilead.avature.net/careers/'
                    ,'http://managementconsultant.co.in'
                  ]
        '''
