...
History : 
          ...
          v1.5 - 13/10/2020 - function copy_file() is updated to return success and failure list and last status 
Pending :
Issues : 
...
def copy_file(src, dst,files_list='',file_suffix='', copy_all_files=False,symlinks=False,ignore=None,add_time_stamp=False,developer_mode = False):
    success = 0 
    erroed = 0 
    status = False
        ...
        for each_file_name in files_list:
            ...
            try:
                if os.path.isdir(s):
                    shutil.copytree(s, d, symlinks,ignore)
                    status = True
                    if developer_mode:print ('copy_folder:\t copy_file :\t File copied !! \n Source : {0} \n Destination : {1}'.format(s,d))
                else:
                    shutil.copy2(s,d)
                    status = True
                ...
                final_text = str(s)+'\t'+str(d)+'error :'+str(s) +'\n'
                write_into_file(file_name = 'copying_errors.txt', contents = add_timestamp(final_text), mode='a')
                erroed+=1
