"""
Functionlaity : Script to git deployment
Version	   : v2.9
History	   :
				v1.0 - 24/09/2020 - initial version
				v2.0 - 25/09/2020 - Fully functioning script
				v2.1 - 28/09/2020 - copy_modified_files() is upgraded to replace more than one file
				v2.2 - 03/10/2020 - option "branch" is added in runtime and ignroed in input format
				v2.3 - 13/10/2020 - logging options are added
				v2.4 - 21/10/2020 - options to deploy the patch files are added and tested
				v2.5 - 28/10/2020 - Branch name can be given in file also
				v2.6 - 29/10/2020 - #1001 - minor upgrade on file ends with '.SQL'
				v2.7 - 30/10/2020 -		 Auto Merge request module is added in this file
				v2.8 - 06/11/2020 -		 regex pattern is updated to deploy the patch files too 
				v2.9 - 12/11/2020 -		 re.IGNORECASE is added in get_modified_files()
...
Pending   items :
			1, Source branch needs to be centralized
...
def get_modified_files(files_list,git_path, developer_mode = False):
	...
	total_ids = re.findall(modified_pattern, str(str_git_status),re.IGNORECASE)
	if developer_mode:
	...
	for each_obj in total_ids:
		try:
			...
			if (current_obj not in modified_list )and [current_obj.lower().endswith(i.lower()) for i in files_list ]:
...
if __name__ == "__main__":
	if False:
		files_directory = 'G:\\Ajith\\Logistics\\2020\\Exception-list-Oct-30-Saahithya\\UT\\Extracted_objects\\modified\\EPE-24432'
		files_list  = ['wm_eitm_sp_save_mlrep_tarzne.sql']
		# copy_modified_files(files_directory,files_list,git_path = GIT_MASTER_PATH,developer_mode= True)
		get_modified_files(files_list =  files_list,git_path = GIT_MASTER_PATH ,developer_mode = True)['git_add_commands'] # runs git status files
	...
	for file_count, each_line in enumerate(input_data):
		...
		print('Processing file : {0}/{1} ... Source ID : {2} Dest ID : {3} , Total files : {4} \n Files : {5},'.format(file_count,len(input_data)-1,epe_id,branch_id,len(files_list),files_list))
		# input('Proceed ??')
	...
	print('Check results at : ',result_file_name)
	end = time.time()
