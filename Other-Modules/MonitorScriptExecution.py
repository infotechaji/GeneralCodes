...
Version : v1.2
History :   
          v1.1 - 06/04/2019 - print statements upgraded to work with python 3 
          v1.2 - 06/18/2019 - Function monitor_script_execution() is updated to send BCP logs in details for tech script
Pending :
Open issues :
"""
...
def get_entered_arguments(python_path='python'):
    count=0
    command=python_path
    if ' ' in command:
        command='"'+str(command)+'"'
    # sys.executable # can be used to get python path 
...
def write_json_file(file_name,result_dict):
...
def read_json(file_name):
    ...
    while True:
        try:
            with open(file_name, 'r') as fpp:
                data = json.load(fpp)
            return data 
        except:
            time.sleep(3)
            try_count+=1
        print ('trying...',try_count)
        if try_count>=3:break
    return data
def write_data_in_file(file_name='processed_accounts.txt',content='',mode='a',add_delimter=True,write_as_json=False):
    with open(file_name,mode) as fp:
        if write_as_json==True:
            #if add_delimter==True:fp.write(json.dumps(content+'\n'))
            #else:fp.write(json.dumps(content))
            fp.write(json.dumps(content, sort_keys=True, indent=4, separators=(',', ': ')))
            return True 
        try:
            if add_delimter==True:fp.write(content+'\n')
            else:fp.write(content)
        except:
            if add_delimter==True: fp.write(str(content)+'\n')
            else:fp.write(str(content))
    return get_processed_accounts(file_name)

def add_processed_accounts(file_name,text,mode='a'):
    with open(file_name,mode) as fp:
        try:
            fp.write(text+'\n')
        except:
            fp.write(str(text.encode('utf-8'))+'\n')
    return get_processed_accounts(file_name)
...
class MonitorScript():
    ...
    def set_time_to_mail(self,time_to_mail=False):
        ...
        timeout=datetime.timedelta(minutes=self.periodic_mail_update)# minutes=1
    ...
    def update_code_execution_status(self,error=''):
        ...
        return False
    def monitor_script_execution(self,delete_current_process_id=False,bcp_total_details='bcp_logs.json'): # Monitors the Entire Script 
            try:
                ...
                if current_python_processes or self.mail_attributes['pending_accounts']==self.mail_attributes['total_accounts']:
                    ...
                    while True:
                        # for i in range(10):
                            # sys.stdout.write(str()+':  Total Active Process :'+str(len(current_python_processes)))
                            # time.sleep(1)
                            # sys.stdout.write('\r')
                        print ('Sleeping for '+str(self.additional_attributes['periodic_mail_update'])+' minutes  !!')
                        for i in range(int(self.additional_attributes['periodic_mail_update'])*60,0,-1):
                            sys.stdout.write('Code is under sleep.. Next update will be in :'+str(i)+'  \r')
                            time.sleep(1)
                        #time.sleep(20)
                        ...
                        try:
                            ...
                            if active_instance_changed or self.time_to_mail :
                                ...
                                if self.mail_attributes['code']=='tech' and self.mail_attributes['total_instances']>1:
                                    #previous logic 
                                    ...
                                    total_directories=self.additional_attributes['thread_dict']['replicated_directory']
                                    print ("total_directories :",total_directories)
                                    if self.developer_mode :
                                        self.log_details(module_name,"total_directories:\t"+str(len(total_directories)))
                                    temp_processed_count=0
                                    #print ('Before looping directory list ')
                                    for each_dir in total_directories:
                                        #print ("each_dir:\t",each_dir)
                                        temp_log_dir=os.path.join(each_dir,self.processed_accounts_log)
                                        if self.developer_mode :
                                            self.log_details(module_name,"temp_log_dir:\t"+temp_log_dir)
                                        temp_processed_count+=int(get_processed_accounts(temp_log_dir))
                                else:
                    ...
                    if not self.additional_attributes['disable_mail']:
                        ...
                        if self.mail_attributes['code']=='tech' and self.mail_attributes['total_instances']>1:
                            ...
                            for each_dir in total_directories:
                                if self.developer_mode :
                                    self.log_details(module_name,"each_dir:\t"+each_dir)
                                print ("each_dir:\t"+each_dir)
                                temp_log_dir=os.path.join(each_dir,bcp_total_details)
                                print ("temp_log_dir :\t",temp_log_dir)
                                temp_data=read_json(temp_log_dir)
                                print ("temp_data :\t",len(temp_data))
                                main_bcp_list.extend(temp_data)
                                
                                if self.developer_mode :
                                    self.log_details("len(main_bcp_list) :",len(main_bcp_list))
                            print ("main_bcp_list :\t",len(main_bcp_list))
                            send_bcp_report_mail(main_bcp_list,receivers=self.additional_attributes['receivers'],additional_attributes={'code':self.mail_attributes['code']})
            except Exception as e: 
                print ("Error- MonitorScript:\t monitor_script_execution: \t",e)
                return False
...
if __name__=="__main__":
    ...
    additional_attributes={
                     'disable_mail':False
                    ,'periodic_mail_update':5 # minutes
                    ,'start_index':0
