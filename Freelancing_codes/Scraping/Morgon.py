...
class Rtrack():
	...
	def get_indivijual_profile(self,page_source ='',developer_mode = False):
		headers = 'Page_count\tData_count\tLink\tName\tTitle\tPhoto\tComplex\tAddress\tDirect\tBranch\tCertifications\tAreas of Focus\tLanguage\tEmail\tOur Story\tServices Include\tView my Bio\tLinkedin_link\tTwitter_link\n'
		...
		while True:
			...
			for each_item in result_list:
				...
				try:
					...
					for each_phone in phones:
						...
						temp_phone  = str(each_item.find("span", {"id": "telephone"}).get_text()).strip()
						if developer_mode:
				...
				if complex_link:
					...
					self.write_into_file(file_name='temp_soup2.txt', content=str(each_item2), mode='w')
					if developer_mode:
						developer_print('Page source extracted :', len(each_item2))
					try:
						our_story = each_item2.find("div", {"class": "Tabs-description"}).get_text().strip()
					except Exception as e:
						if developer_mode:
					...
					try:
						...
						for bio_index,each_bio in enumerate(total_bio_team):
							bio_name = each_bio.find("div", {"class": "Bio-nameText"}).get_text().strip()
							if developer_mode:
							...
							if name.lower() in bio_name.lower():
								bio = each_bio.find("div", {"class": "BioAbout-row BioAbout-row--main"}).get_text().strip()
								...
								if developer_mode:
									developer_print('Bio is extraced :', bio)
								try:
						...
						if developer_mode:
							developer_print('Final email id ', len(email))
					except Exception as e:
						if developer_mode:
							developer_print('Error in getting Bio /email  :', e)
						pass
				...
				final_text = [page_count,data_count,res['complex_link'],res['name'],res['image_link'],res['complex_data'],res['title'],res['address']
							,res['direct_phone'],res['branch_phone'],res['certification'],res['area_of_focus'], res['languages'],
						  res['email'], res['our_story'], res['services'], res['bio'],res['linkedin_link'],res['twitter_link_link']
						  ]
			...
			for res in result_set_to_return:
				...
				final_text_out = [page_count,data_count,res['complex_link'], res['name'], res['image_link'], res['complex_data'], res['title'],
						 res['address'], res['direct_phone'], res['branch_phone'], res['certification'], res['area_of_focus'],
						 res['languages'],res['email'], res['our_story'], res['services'], res['bio'], res['linkedin_link'],res['twitter_link_link']
						 ]
