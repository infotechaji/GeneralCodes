...
def get_file_name_with_time_stamp(file_name,check_file_existence=True,include_directory_name=True,skip_timestamp=False):
    if not file_name: return False
    if check_file_existence and (not os.path.isfile(file_name)): return False
    timestamp_is=get_time_stamp_for_file()
...
def rename_file_to_time_stamp_prefix(file_name,return_file_name=False):
    if file_name:
        file_name_is=file_name
    else:
        return False
    if os.path.isfile(file_name_is):
        ...
        new_file_name_is= get_file_name_with_time_stamp(file_name_is,check_file_existence=False)
        try:
