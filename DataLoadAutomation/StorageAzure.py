"""
    ...
    History    :
                ...
                2.2 - 08/04/2017 - Return error message for upload_file
                2.3 - 08/07/2017 - List blobs for entries more than 5000(default limit)
"""
...
class PythonAZURE():
    ...
    def upload_file(self, sub_folder, file_path, filename_to_save=None,new_extension=None):
        try:
            if os.path.isfile(file_path):
                ...
                return 'File does not exists:' + repr(file_path)
    ...
    def download_file(self, azure_file_path,path):##yet to test
        try:
            ...
            return False
    def delete_file(self, azure_blob_name):##yet to test
        try:
            #return True
            if self.developer_mode: self._print_('delete_file: deleting -' + str(azure_blob_name))
            self.conn.delete_blob(self.bucket,azure_blob_name)
            return True
        except:
            if self.developer_mode: self._print_ ( 'Error while list all the files from the bucket \''+ str(self.bucket) + '\'. Error is '  + str(sys.exc_info())  + '\t. Connection status is ' + str(self.connection_status) + '. Bucket set status is ' + str(self.bucket_status))
            return False
    def download_all_files(self,target_folder,filter_by=None):
        try:
            all_file_name=self.list_all_files(filter_by=filter_by)
            ...
            return False
    def delete_all_files(self,target_folder,filter_by=None):
        try:
            ...
            for each_file in all_file_name:
                ...
                if '/' in target_file_name:
                    ...
                    pass
                # self.download_file(each_file,os.path.join(target_folder,target_file_name))#+'.psql'))
                self.delete_file(each_file)#,os.path.join(target_folder,target_file_name))#+'.psql'))
                # self.download_file(each_file,os.path.join(target_folder,target_file_name))
            all_file_name=self.list_all_files(filter_by=filter_by)
            self._print_('After deletion,no of files in the bucket '+str(self.bucket)+' '+str(len(all_file_name)))
            return True
        except:
            if self.developer_mode: self._print_ ( 'Error while Downloading all the files from the bucket \''+ str(self.bucket) + '\'. Error is '  + str(sys.exc_info())  + '\t. Connection status is ' + str(self.connection_status) + '. Bucket set status is ' + str(self.bucket_status))
            return False
    def list_file(self,filter_by=None):#Limit is 5000 blobs maximum. Use list_all_files for more than 5000 entries.
        try:
            list_of_files=[]
            if filter_by:
            ...
            if self.developer_mode: self._print_ ( 'Error while list all the files from the bucket \''+ str(self.bucket) + '\'. Error is '  + str(sys.exc_info())  + '\t. Connection status is ' + str(self.connection_status) + '. Bucket set status is ' + str(self.bucket_status))
            return False
    def list_all_files(self,filter_by=None):
        try:
            list_of_all_files=[]
            if filter_by:
                if len(filter_by.strip()) <3:
            ...
            while True:
                ...
                list_of_keys = self.conn.list_blobs(self.bucket,marker=marker)
                for each_result in list_of_keys:
                    if filter_by:
                        if each_result.name.startswith(filter_by):
                            list_of_all_files.append(each_result.name)
                    else:
                        list_of_all_files.append(each_result.name)
                
                if list_of_keys.next_marker:
                    marker=list_of_keys.next_marker
                else:
                    break
            return list_of_all_files
        except:
    ...
    def list_file_size(self,filter_by=None):#size need to be added
        try:
            ...
            return False
    def DONOTUSE_file_size(self,key_name):
        try:
    ...
    def get_file_properties(self):
        ...
        #blob.properties.__dict__ -->{'content_length': 99831, 'blob_type': 'BlockBlob', 'append_blob_committed_block_count': None,
        #                            'last_modified': datetime.datetime(2016, 11, 23, 5, 46, 10, tzinfo=tzutc()), 'content_range': None,
        #                            'etag': '"0x8D4136407173436"', 'page_blob_sequence_number': None, 
        #                            'content_settings': <azure.storage.blob.models.ContentSettings object at 0x7f4f8f870510>,
        #                            'copy': <azure.storage.blob.models.CopyProperties object at 0x7f4f8f870650>,
        #                            'lease': <azure.storage.blob.models.LeaseProperties object at 0x7f4f8f870810>}
if __name__ == '__main__':
    if not True:
        ...
        azure=PythonAZURE('fiindhdinsight','DV5LBbma0JAekanA3OuUCol6c3EoULWD9sp08IPBzBNmwu7o3lvbjRONxBvL9qxRWFPO+Tmkz6VjWDExVvz0XQ==')
        azure.set_bucket_name('campaigninput')
