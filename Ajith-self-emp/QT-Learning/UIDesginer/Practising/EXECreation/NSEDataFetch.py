...
def download_bhav_copy(start_date,end_date,data_requirements= ['company_wise'],output_format = 'zip',developer_mode= False,output_directory=os.getcwd(),selected_companies = [],overrite = False,progressBar = ''): # date_requirement= ['company_wise','date_wise']  , output_format = 'single_files'
    ...
    for ind,each_date in enumerate(res):
        if progressBar:
            temp_val = int(((ind+1)/len(res))*100)
            progressBar.setValue(temp_val)
        ...
        try:
            ...
            # elif 'date_wise' in data_requirements:
